...
def listComponents(
    ...
    if url_:
        ...
        print(f"Getting list of components for {board_}, {variant} from {bm.buildManifestPath}")
...
def listComponent(
    ...
    if variant_ > 0:
        variant = "Update"
    if url_:
        print(f"Getting path of component {component_} for {board_}, {variant} from {bm.url}")
    else:
        print(f"Getting path of component {component_} for {board_}, {variant} from {bm.buildManifestPath}")
    path = bm.getComponentPath(board=board_, component=component_, update=bool(variant_))
...
def downloadComponent(
    ...
    try:
        ...
        if url_ and len(url_) > 0:
            ...
            raise click.BadParameter(f"Failed to initialize BuildManifest tool: {manifest_}")
    variant = "Erase"
    if variant_ > 0:
        variant = "Update"
    if url_:
        print(f"Downloading component {component_} for {board_}, {variant} from {bm.url}")
    else:
        print(f"Downloading component {component_} for {board_}, {variant} from {bm.buildManifestPath}")
    path = bmt.downloadComponent(board=board_, component=component_, update=bool(variant_))
    if not path or len(path) < 1:
